...
const NAME_REGEX = /^([A-Za-záéíóúÁÉÍÓÚñÑ]{2,16}\s){1}[A-Za-záéíóúÁÉÍÓÚñÑ]{2,16}$/;
...
list.addEventListener('click', (e) => {
  ...
  if (e.target.closest('.edit-icon') || e.target.querySelector('.edit-icon')){
    ...
    const input = inputs.children[0];// selecciono el nombre
    const phone = inputs.children[1];// selecciono el numero de telefono
    const editIcon = e.target.closest('.edit-icon'); // selecciono el boton de editar
    function validation() {
    ...
    if (editIcon.classList.contains('editando')){
        ...
        //procedo a guardar en localStorage
        localStorage.setItem('listaContactos',list.innerHTML)
      }else{
